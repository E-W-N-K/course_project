...
export const UIForm = ({
  ...
  const checkFormValidity = () => {
    const validRefs = inputRefs.current.filter((ref): ref is UIInputRef => ref !== null)
    const allValid = validRefs.length === 0 || validRefs.every(ref => ref.isValid())
  ...
  const childrenWithRefs = Children.map(children, (child) => {
    if (isValidElement(child)) {
      ...
      if (childType?.displayName === 'UIInput') {
        ...
        const originalOnChange = (child.props as { onChange?: (value: string, error?: string) => void }).onChange
  ...
  const renderedActions = typeof actions === 'function' ? actions(isFormValid) : actions
