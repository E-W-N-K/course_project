...
interface UIInputProps extends Omit<InputHTMLAttributes<HTMLInputElement>, 'onChange'> {
...
const validateInput = (value: string, rules?: ValidationRules, customValidate?: (value: string) => string | undefined): string | undefined => {
...
const UIInputComponent = forwardRef<UIInputRef, UIInputProps>(({
  ...
  return (
    <div className={`${styles.wrapper} ${className}`}>
      ...
      />
      {displayError && (
        <span className={styles.error}>{displayError}</span>
      )}
    </div>
  )
})
